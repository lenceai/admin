...
Usage examples:
  python ilo_power_1.0.3.py -i 10.0.0.100 -u Administrator -s
  python ilo_power_1.0.3.py -f servers.csv -s
  python ilo_power_1.0.3.py -f servers.csv --power-watts
  python ilo_power_1.0.3.py -f servers.csv --monitor-power --interval 15
  python ilo_power_1.0.3.py -f servers.csv --power-on

Version: 1.0.3 (redfish based)

Requires: pip install redfish pandas
"""
...
def get_system_status(system, detailed=False, debug=False):
    ...
    with session_manager as client:
        ...
        try:
            ...
            if detailed:
                ...
                # Basic status line
                watts_str = f"{result['watts']}W" if result['watts'] is not None else "Unknown"
                # Basic format: IP | Model | S/N | Power | Watts | Health | iLO FW
                print(f"{ip} | {result['model']} | {result['identifier']} | Pwr: {result['power_state']} | Use: {watts_str} | Health: {result['health']} | iLO: {result['ilo_version']}")
...
def main():
    ...
    try:
        ...
        version_info = []
        version_info.append(f"HPE iLO Power Management Script v1.0.3 (redfish based with NumPy)")
        ...
        if args.status:
            ...
            # Using ThreadPoolExecutor for potentially faster status checks on many systems
